...
do 
    ...
    do
        echo "Fold=${fold}"
        python3 -u src/run_g_or_gxe_model_old.py --cv=${cv} --fold=${fold} --seed=${seed} --model=GxE --A --svd --lag_features > "logs/gxe_model_A_svd_cv${cv}_fold${fold}_seed${seed}_lag_features.txt"
        echo '[GxE] A model ok'
